...
\begin{algorithmic}[1]
\State \textbf{Entrada:} $\mathbf{x}_0$ ley a priori para los estados del sistema, $\mathbf{p}_0$ ley a priori para los parámetros sujetos a estimación, $\mathbf{y}$ observaciones del sistema, $\text{iters}$ la cantidad de iteraciones.
\State \textbf{Salida:} $\mathbf{\hat{p}} \in \R^{(\text{iters}+1)\times n_p}$ estimación de los parámetros en cada iteración.
\State $\hat{\mathbf{p}}_{0,:} \gets \text{Media}(\mathbf{p}_0)$.
\State $\hat{\mathbf{x}}_{0} \gets \text{Media}(\mathbf{x}_0)$.
\State $\Tilde{\mathbf{x}} \gets (\hat{\mathbf{x}}_{0}, \hat{\mathbf{p}}_{0,:})$
\For{k=1, \dots, \text{iters}} \\
    \quad $\hat{\mathbf{z}} \gets $ KKKF($\Tilde{\mathbf{x}}, \mathbf{y})$. \\
    \quad $\hat{\mathbf{p}}_{k,:} \gets \hat{\mathbf{z}}_{-1,n_p:}$ \Comment{Última iteración del filtro en las últimas $n_p$ entradas.} \\
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=\linewidth]{img/content/chapter4/error_vs_N_01.pdf}
    \caption{Caption}
    \label{fig:enter-label}
\end{figure}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=\linewidth]{img/content/chapter4/error_vs_N_001.pdf}
    \caption{Caption}
    \label{fig:enter-label}
\end{figure}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=\linewidth]{img/content/chapter4/error_vs_N_025.pdf}
    \caption{Caption}
    \label{fig:enter-label}
\end{figure}

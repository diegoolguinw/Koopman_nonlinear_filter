...
\begin{teo}
    Sea $\delta \in (0, 1)$, si $\U \H_X \subset \H_X$, $\G \R^p \subset \H_X$, $\H_\X$ es equivalente en norma a $H^{\nu + n/2}$si $\H_\X$ es equivalente en norma $H^{\nu + n/2}$ y
    \[
...
\begin{teo}[Error de kerKKF]
   Sea $\delta \in (0, 1)$, si $\U \H_X \subset \H_X$, $\G \R^p \subset \H_X$, $\B \R^n \subset \H_X$, $\H_\X$ es equivalente en norma $H^{\nu + n/2}$ y
    \[
    8\|k\|^2_\infty \ln(4/\delta) \leq \frac{1}{2} (N+1)^{-7/2},
    \]
...
\begin{equation*}
    ...
    \end{pmatrix},
\end{equation*}
tenga rango completo, donde $n$ es la dimensión del estado, en este caso $n=3$. 
...
\begin{table}[h!]
    \centering
    \caption{Estimación de parámetros del modelo SIR con recuperación (IC 95\%).} 
    \begin{tabular}{|c|c|c|c|c|}
    \hline
    \textbf{Parámetro}& \textbf{Real} & \textbf{DEMetropolisZ} & \textbf{NUTS} & \textbf{kerKKF}  \\ \hline
...
\hline
\end{tabular}
    \caption{Métricas para trayectorias \textit{sampleadas} del modelo SIR con pérdida de inmunidad.}
    \label{tab:metricas_traj_SIR_rec}
\end{table}
